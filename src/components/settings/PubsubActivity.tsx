...
function StatusAlert({
  ...
  const Icon =
    IconComp ?? (kind === "success" ? CheckCircle2 : kind === "info" ? Info : AlertTriangle);
  return (
    <div
      className={`flex flex-col gap-3 rounded-md border ${s.border} ${s.bg} p-3 md:flex-row md:items-start md:justify-between`}
    >
...
function StatCard({
  ...
  const tone = accent === "danger" ? "text-destructive" : accent === "warn" ? "text-amber-600" : "";
  return (
...
function KV({ k, v, mono, bad }: { k: string; v: React.ReactNode; mono?: boolean; bad?: boolean }) {
  return (
    <div className="min-w-0">
      ...
      <div className={`${mono ? "font-mono" : ""} ${bad ? "text-destructive" : ""} break-all`}>
        {v}
      </div>
    </div>
  );
  ...
  switch (s.kind) {
    ...
    case "live":
      return (
        <Badge
          variant="outline"
          className="h-5 border-emerald-500/40 bg-emerald-500/10 text-[10px] font-normal text-emerald-700 dark:text-emerald-400"
        >
    ...
    case "stale_amber":
      return (
        <Badge
          variant="outline"
          className="h-5 border-amber-500/40 bg-amber-500/10 text-[10px] font-normal text-amber-700 dark:text-amber-400"
        >
    ...
    case "stale_red":
      return (
        <Badge variant="destructive" className="h-5 text-[10px] font-normal">
          {s.ageHours >= 24
            ? `${Math.round(s.ageHours / 24)}d stale`
            : `${Math.round(s.ageHours)}h stale`}
        </Badge>
      );
  }
        ...
        {(["p50", "p95", "p99"] as const).map((k) => {
          ...
          return (
            <div key={k}>
              <div className="text-[10px] uppercase tracking-wide text-muted-foreground">{k}</div>
              <div
                className={`mt-1 text-lg font-semibold tabular-nums ${LATENCY_TONE_CLASS[tone]}`}
              >
                {fmtLatency(v)}
              </div>
            </div>
      ...
      <div className="space-y-6 p-4 md:p-6">
            ...
            <div className="space-y-1.5 rounded-md border p-3">
              ...
              {stuckJobs.map(
                (j: {
                  ...
                  <div
                    key={j.id}
                    className="flex items-center justify-between gap-2 rounded border bg-card p-2 text-xs"
                  >
                    <div className="min-w-0 flex-1">
                      <div className="truncate font-medium">
                        {j.subject ?? "(no subject yet)"}{" "}
                        <span className="font-normal text-muted-foreground">
                          — {j.from_addr ?? "unknown sender"}
                        </span>
                      </div>
                    ...
                    >
                      <RefreshCw
                        className={`mr-1 h-3 w-3 ${retryingJob === j.id ? "animate-spin" : ""}`}
                      />
                      Force retry
                    </Button>
                  </div>
        ...
        {stats && (
          <section>
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
              ...
              <StatCard
                label="Errors"
                value={stats.errors24}
                accent={stats.errors24 > 0 ? "danger" : undefined}
              />
            </div>
        ...
        <section>
          <div className="mb-3 flex items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              ...
              <StalenessBadge
                ...
                sampleCount={latencyQ.data?.push_to_ack?.count ?? 0}
              />
            </div>
          ...
          <p className="mt-2 text-[11px] text-muted-foreground">
            Targets: p50 &lt; 1s (green), p50 &lt; 3s (amber). Anything beyond suggests the worker
            queue is backed up or Gmail API is slow.
            {latencyQ.data?.push_to_ack?.count === 0 &&
              latencyQ.data?.push_to_visible?.count === 0 && (
                <>
                  {" "}
                  Latency telemetry populates on every real Pub/Sub push — &quot;no samples&quot;
                  means push hasn&apos;t fired in the lookback window.
                </>
              )}
            {"error" in (latencyQ.data ?? {}) && (
              <span className="text-destructive">
                {" "}
                · RPC error: {(latencyQ.data as { error?: string }).error}
              </span>
            )}
          </p>
        </section>
              ...
              <CollapsibleContent>
                <div className="space-y-2 border-t p-3 text-xs">
                  <div className="grid gap-2 md:grid-cols-2">
                    <KV
                      k="emailAddress (decoded)"
                      v={lastPush.email_address ?? "(missing)"}
                      bad={!lastPush.email_address}
                    />
                    <KV
                      k="historyId (decoded)"
                      v={lastPush.history_id ?? "(missing)"}
                      bad={!lastPush.history_id}
                    />
                    <KV
                      k="accounts_matched"
                      v={String(lastPush.accounts_matched ?? 0)}
                      bad={(lastPush.accounts_matched ?? 0) === 0}
                    />
              ...
              <div className="space-y-4 border-t p-3 text-xs">
                  ...
                  <StatusAlert
                      ...
                      <>
                        ...
                        {(() => {
                          const lastPushMs = lastPush
                            ...
                            : 0;
                          const lastRenewMs = lastRenew
                            ? new Date(lastRenew.received_at).getTime()
                            : 0;
                          const verified =
                          ...
                          return verified ? (
                            <span className="ml-2 text-emerald-700 dark:text-emerald-400">
                              · verified by a fresh matched push ✓
                            </span>
                          ) : null;
                        })()}
                      </>
                    ...
                    body={
                      <>
                        ...
                        <div className="mt-1">
                          <strong>How to verify:</strong> send yourself an email from another
                          account and watch this panel for ~30s. A new{" "}
                          <span className="font-mono">push</span> row with{" "}
                          <span className="font-mono">accounts_matched ≥ 1</span> means real-time
                          push is working.
                        </div>
                      </>
                    }
                ...
                <div>
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    {(["empty", "realistic"] as const).map((mode) => (
                      <Button
                        ...
                        onClick={async () => {
                          ...
                          try {
                            ...
                            setPingResult(r);
                            if (r.ok)
                              toast.success(`Webhook reachable (${r.status} in ${r.elapsed_ms}ms)`);
                      ...
                      >
                        <RefreshCw
                          className={`mr-2 h-4 w-4 ${pinging === mode ? "animate-spin" : ""}`}
                        />
                    ...
                    {pingResult && (
                      <div className="text-xs text-muted-foreground">
                        <span className={pingResult.ok ? "text-foreground" : "text-destructive"}>
                          {pingResult.ok ? "✓" : "✗"} {pingResult.status} · {pingResult.elapsed_ms}
                          ms
                        </span>
                        {pingResult.mode && (
                          <>
                            {" "}
                            · mode: <span className="font-mono">{pingResult.mode}</span>
                          </>
                        )}
                        {pingResult.account_email && <> · {pingResult.account_email}</>}
                        {" · "}
                        topic:{" "}
                        {pingResult.topic_set ? (
                          "set"
                        ) : (
                          <span className="text-destructive">not set</span>
                        )}
                        {pingResult.error && (
                          <span className="text-destructive"> · {pingResult.error}</span>
                        )}
                      </div>
                    )}
                  </div>
                ...
                <div className="rounded border bg-card p-3">
                  <div className="text-[10px] uppercase tracking-wide text-muted-foreground">
                    GCP Pub/Sub setup checklist
                  </div>
                  <ol className="mt-2 list-decimal space-y-1 pl-5 text-muted-foreground">
                    <li>
                      Topic must equal{" "}
                      <span className="font-mono">
                        {diag?.pubsubTopic ?? "(GMAIL_PUBSUB_TOPIC not set)"}
                      </span>
                      {diag?.pubsubTopic && (
                        <>
                          {" "}
                          <CopyBtn value={diag.pubsubTopic} />
                        </>
                      )}
                    </li>
                    <li>
                      ...
                      {diag?.webhookUrl && (
                        <>
                          {" "}
                          <CopyBtn value={diag.webhookUrl} />
                        </>
                      )}
                    </li>
                    <li>
                      <span className="font-mono">gmail-api-push@system.gserviceaccount.com</span>{" "}
                      needs <span className="font-mono">roles/pubsub.publisher</span> on the topic.
                    </li>
                    <li>Subscription must not be paused; ack deadline ≥ 10s.</li>
                    <li>
                      The Gmail watch must be re-armed against the same topic (use the button
                      above).
                    </li>
                  </ol>
                </div>
...
function LastPushStaleBadge({
  lastPush,
  lastRenew,
}: {
  lastPush: { received_at: string };
  lastRenew: { received_at: string } | null;
}) {
