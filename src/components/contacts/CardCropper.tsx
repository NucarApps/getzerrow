...
import { autoDetectRect, centeredFallbackRect, type Rect } from "@/lib/ui/card-detect";
...
export function CardCropper({ src, onConfirm, onCancel }: Props) {
  ...
  function handleImgLoad() {
    ...
    if (!ctx) {
      setRect(centeredFallbackRect(w, h));
      return;
    }
  ...
  function reset() {
    if (!imgSize) return;
    setRect(centeredFallbackRect(imgSize.w, imgSize.h));
  }
