      ...
      POST: async ({ request }) => {
        ...
        return withCronRun("gmail-poll", async ({ runId }) => {
          const { data: accounts, error } = await supabaseAdmin
            .from("gmail_accounts")
            .select(
              "id, email_address, watch_expiration, last_push_at, created_at, needs_reconnect",
            );
          if (error) {
          ...
          for (const acc of accounts ?? []) {
            ...
            const accountAgeMs = acc.created_at
              ? Date.now() - new Date(acc.created_at).getTime()
              : 0;
            const accountSilent =
              lastPushMs !== null
            ...
            const watchActive =
              acc.watch_expiration && new Date(acc.watch_expiration).getTime() > Date.now();
              ...
              try {
                ...
                if (w) {
                  ...
                  try {
                    ...
                    logError(
                      "poll.pubsub_log_failed",
                      { run_id: runId, account_id: acc.id, kind: "watch_rearm_auto" },
                      e,
                    );
                  }
                }
