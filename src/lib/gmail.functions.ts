...
import { backfillRecent, backfillWindow, syncSinceHistory, learnFromLinkedLabel, reconcileLocalInbox, loadOlderFromLabel, startBackfillJob, cancelBackfillJob, invalidateAccountContext, invalidateAccountContextForUser } from "./sync.server";
import {
  listLabels,
  createLabel,
  modifyMessage,
...
// Diagnostics functions live in a sibling file; re-export here so existing
...
export {
  ...
  runJobsNow,
  enqueueGmailMessage,
} from "./gmail-diagnostics.functions";
...
export const connectGmailFromSession = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // encrypted with pgsodium AEAD before they touch the table.
