...
//
// These snapshots pin the EXACT string `collapseRunawayRepeats` produces for a
// set of representative inputs (STT hallucination loops plus normal speech). If
// a change to the sanitizer alters any output, this test fails in CI, forcing a
// deliberate review + snapshot update rather than a silent behavior change.
//
...
describe("transcript sanitizer — golden output regression", () => {
  it("collapses an alternating two-sentence loop (the iOS bug pattern)", () => {
    ...
    expect(collapseRunawayRepeats(input)).toBe("Why are we doing this later? Okay, hold on.");
  });

  it("collapses a single sentence repeated many times", () => {
    const input = REPEAT("Thank you. ", 30);
    expect(collapseRunawayRepeats(input)).toBe("Thank you.");
  });

  it("collapses a three-sentence block loop", () => {
    const input = REPEAT("First point. Second point. Third point. ", 10);
    expect(collapseRunawayRepeats(input)).toBe("First point. Second point. Third point.");
  });
  ...
  it("leaves a normal transcript untouched", () => {
    ...
    expect(collapseRunawayRepeats(input)).toBe(input);
  });
  ...
  it("collapses a leading loop while preserving the real tail", () => {
    ...
    expect(collapseRunawayRepeats(input)).toBe(
