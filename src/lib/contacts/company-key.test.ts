...
describe("deriveCompanyKey", () => {
  it("collapses fragmented company rows into one key", () => {
    ...
    const keys = ["c1", "c2", "c3"].map(
      (company_id) =>
        deriveCompanyKey(contact({ company_id }), ctx({ companiesById }))?.key,
    );
  ...
  it("keeps dealers separate from the factory brand", () => {
    ...
    const factory = deriveCompanyKey(
      contact({ company_id: "c1" }),
      ctx({ companiesById }),
    );
    const dealer = deriveCompanyKey(
      contact({ company_id: "c2" }),
      ctx({ companiesById }),
    );
  ...
  it("derives from free-text company when no entity is linked", () => {
    const derived = deriveCompanyKey(
      contact({ company: "Nissan North America" }),
      EMPTY_CTX,
    );
    expect(derived?.key).toBe("nissan");
  ...
  it("falls back to a pretty domain name for unknown domains", () => {
    const derived = deriveCompanyKey(
      contact({ email: "someone@nissan-usa.com" }),
      EMPTY_CTX,
    );
  ...
  it("ignores personal email domains", () => {
    expect(
      deriveCompanyKey(contact({ email: "someone@gmail.com" }), EMPTY_CTX),
    ).toBeNull();
  });
});
