...
import { emailDomain, isPersonalDomain, isRoutableDomain } from "@/lib/company-domains";
...
${corpus}`;
    ...
    for (const c of candidates) {
      ...
      // isRoutableDomain replaces the dot check emailDomain used to do inline;
      // without it a dotless host becomes a junk suggested company name.
      if (!domain || !isRoutableDomain(domain) || isPersonalDomain(domain)) continue;
