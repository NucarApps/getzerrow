...
/** Throw if the target group is an auto-generated company subgroup — those
 *  are managed rows and can only be affected by changes to their parent's
 *  members / the parent's toggle. */
async function assertNotAutoGenerated(
  supabase: DB,
  groupId: string,
  action: string,
): Promise<void> {
  ...
  if (data?.auto_generated_from_group_id) {
    throw new Error(
      `Can't ${action}: this subgroup is managed automatically from its parent group's contacts`,
    );
  }
}
const MAX_DEPTH = 4;
...
const GROUP_SELECT =
  "id,name,color,kind,created_at,folder_id,carddav_uid,updated_at,parent_group_id,auto_company_subgroups,auto_generated_from_group_id";
...
export const createContactGroup = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator((d: { name: string; color?: string; parent_group_id?: string | null }) =>
    z
      .object({
...
export const updateContactGroup = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator((d: { id: string; name?: string; color?: string; parent_group_id?: string | null }) =>
    z
      .object({
        id: z.string().uuid(),
  ...
  .handler(async ({ data, context }) => {
    ...
    await assertNotAutoGenerated(supabase, id, "edit");
...
export const deleteContactGroup = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await assertNotAutoGenerated(supabase, data.id, "delete");
...
export const setContactGroups = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator((d: { contactId: string; groupIds: string[] }) =>
    z
      .object({
...
export const addContactsToGroups = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator((d: { groupIds: string[]; contactIds: string[] }) =>
    z
      .object({
  ...
  .handler(async ({ data, context }) => {
    ...
    for (const gid of data.groupIds) {
      await assertNotAutoGenerated(supabase, gid, "add contacts to");
    }
...
export const linkContactGroupToFolder = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator((d: { groupId: string; folderId: string | null }) =>
    z
      .object({
