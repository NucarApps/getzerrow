...
// Stale-lease window. A pull+push run can now use up to PUSH_WALL_BUDGET_MS
// (~55s) to drain first-time backlogs, so a healthy run may take up to ~75s
// including pull + finalize. If a row's `locked_at` is older than this window,
// the previous worker was killed before finalize (fetch abort, Worker CPU
// cap) — safe to reclaim so the next tick isn't rejected with "locked" and
