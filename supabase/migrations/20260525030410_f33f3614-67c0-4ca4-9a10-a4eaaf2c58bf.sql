DROP FUNCTION IF EXISTS public.claim_message_jobs(integer, integer);
CREATE OR REPLACE FUNCTION public.claim_message_jobs(p_limit integer, p_priority integer DEFAULT NULL::integer)
 RETURNS TABLE(id uuid, gmail_account_id uuid, gmail_message_id text, user_id uuid, attempt integer, priority smallint, published_at_ms bigint)
 LANGUAGE plpgsql
 SECURITY DEFINER
