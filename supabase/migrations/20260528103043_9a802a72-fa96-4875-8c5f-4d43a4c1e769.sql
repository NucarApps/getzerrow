-- Phase 3 Migration A: add batch-decrypt RPCs + stop writing plaintext into
-- columns scheduled for drop. The plaintext columns still EXIST after this
-- migration — Migration B drops them once code is on the new helpers.

-- 1. New batch-decrypt RPC for inbox list rows (small fields only)
...
-- 2. New batch-decrypt RPC for contacts list rows
CREATE OR REPLACE FUNCTION public.get_contacts_list_fields_decrypted(p_ids uuid[], p_key text)
RETURNS TABLE(id uuid, relationship_summary text, phone text)
LANGUAGE sql STABLE SECURITY DEFINER
SET search_path = public, private, extensions
AS $$
  SELECT
...
-- 3. Key-aware claim_forward_retries that decrypts body_text on the fly.
-- Keep the old signature for now; forward-retry will call the new one.
CREATE OR REPLACE FUNCTION public.claim_forward_retries_v2(p_limit integer, p_key text)
RETURNS TABLE(id uuid, gmail_account_id uuid, gmail_message_id text, folder_id uuid,
              subject text, from_addr text, from_name text, body_text text, snippet text,
              received_at timestamp with time zone, forward_attempts smallint)
...
BEGIN
  ...
  UPDATE public.emails e
   ...
   RETURNING
     e.id, e.gmail_account_id, e.gmail_message_id, e.folder_id,
     ...
     e.received_at, e.forward_attempts;
END;
$$;

-- 4. Rewrite write RPCs to stop populating plaintext columns being dropped.
-- We still maintain the small plaintext fields kept under Option B (subject,
-- snippet, from_name, to_addrs, cc) for indexing & filtering.
...
BEGIN
  ...
  IF p_subject IS NOT NULL OR p_snippet IS NOT NULL OR p_body_text IS NOT NULL THEN
    SELECT user_id, subject, snippet INTO v_user_id, v_subject, v_snippet
...
CREATE OR REPLACE FUNCTION public.set_reply_draft_encrypted(p_user_id uuid, p_email_id uuid, p_draft_text text, p_key text)
RETURNS uuid
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = public, private, extensions
AS $$
...
CREATE OR REPLACE FUNCTION public.set_contact_encrypted_fields(p_contact_id uuid, p_notes text, p_relationship_summary text, p_address_line1 text, p_address_line2 text, p_phone text, p_key text)
RETURNS void
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = public, private, extensions
AS $$
BEGIN
  UPDATE public.contacts SET
    ...
    summary_generated_at     = CASE WHEN p_relationship_summary IS NULL THEN summary_generated_at     ELSE now() END,
    updated_at               = now()
