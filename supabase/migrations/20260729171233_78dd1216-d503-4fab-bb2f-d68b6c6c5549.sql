...
CREATE OR REPLACE FUNCTION public.get_emails_list_decrypted(p_account_id uuid, p_user_id uuid, p_scope text, p_folder_id uuid, p_cursor timestamp with time zone, p_limit integer, p_key text)
 RETURNS TABLE(id uuid, from_addr text, from_name text, subject text, snippet text, to_addrs text, ai_summary text, classification_reason text, received_at timestamp with time zone, is_read boolean, is_archived boolean, folder_id uuid, ai_confidence real, thread_id text, classified_by text, matched_filter_ids uuid[], matched_folder_ids uuid[], has_attachment boolean, processed_at timestamp with time zone, raw_labels text[], snoozed_until timestamp with time zone, gmail_message_id text, surfaced_to_inbox boolean, origin_addr text, reply_to_addr text, is_forwarded boolean)
 LANGUAGE sql
...
AS $function$
  SELECT
    ...
    e.surfaced_to_inbox,
    e.origin_addr, e.reply_to_addr, e.is_forwarded
